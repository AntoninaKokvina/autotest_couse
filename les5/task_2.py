...
def repeats(our_str):
    """
    функция добавляет к каждому символу строки его количество вхождений
    :param our_str: исходная строка
    :return: строка, измененная по условию
    """

    our_lst = [x for x in our_str]  # раскладываем исходную строку в список
    
    # начиная с конца списка для каждого элемента считаем кол-во вхождений и дописываем его к элементу
    for i in range(len(our_lst) - 1, -1, -1):
        our_lst[i] = our_lst[i] + '_' + str(our_lst.count(our_lst[i]))

    new_str = ''.join(our_lst)
    return new_str
